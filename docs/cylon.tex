...
Here we are concerned with the problem of termining if two objects have collided. If the objects are made of vertices and faces the possible collisions can be vertex-vertex, vertex-edge, verted-face, edge-face, face-face. Most of them can be ignored but vertex-face and edge-edge.
...
Finally we loop over all faces of $A$, and vertices of $B$, we run the above algotithm and if a collision occurs we resolve the penetration, we compute the velocity of the collision point and we appli the impulse at collision point as discussed in the previous section.
