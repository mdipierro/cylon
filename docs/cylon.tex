...
A rotation of the angle $\theta$ around an arbitrary direction $\hat n = (n_0,n_0,n_1)$ is given by:

\begin{equation}
R_{\hat n}(\theta) \equiv \left(\begin{tabular}{ccc} 
$2(n^2_0 s^2 + c^2)-1 $ & $
  2(n_0 n_1 s^2 - n_2 s c) $ & $ 2(n_0 n_2 s^2 - n_1 s c)$ \\ 
$2(n_0 n_1 s^2 + n_2 s c) $ & $ 2(n^2_1 s^2 +c^2)-1
  $ & $ 2(n_1 n_2 s^2-n_0 s c)$ \\ 
$2(n_0 s c - n_1 s c) $ & $ 2(n_0 s c + n_1 n_2 s^2) $ & 
  $ 2(n^2_2 s^2 + c^2)-1$
\end{tabular}\right)
\end{equation}
...
\begin{equation}
\tilde \theta = (\theta_0,\theta_1,\theta_2,\theta_3) = (n_0 \sin(\theta/2), n_1 \sin(\theta/2), n_2 \sin(\theta/2),\cos(\theta/2))
\label{quaternion}
\end{equation}
...
where $(\beta_0,\beta_1,\beta_2)=(\beta n_0,\beta n_1,\beta n_2)$
...
We now consider a rotation propotional to time, $t$. In particular a rotation around a direction $\hat n=(n_0,n_1,n_2)$ of an angle $\theta_t = \omega t$ ($\theta$ depends on $t$). Repeating the previous steps and replacing $\theta_t$ for $\theta$ in eq.~\ref{quaternion} we obtain:
\begin{equation}
\tilde \theta_t \equiv (n_0 \sin(\omega t/2), n_1 \sin(\omega t/2), n_2 \sin(\omega t/2),\cos(\omega_t/2))
\end{equation}
...
where
\begin{equation}
\vec \omega \equiv (\omega n_0, \omega n_1, \omega n_2)
\end{equation}
...
R_t = & R(\omega_dt dt) R_t & \qquad \textrm{(update orientation)}\\
...
\begin{eqnarray}
\vec v'_A &=& \vec v_A + \frac{m_B}{m_A+m_B}(c+1)(\vec v_B-\vec v_A) \\
\vec v'_B &=& \vec v_B + \frac{m_A}{m_A+m_B}(c+1)(\vec v_A-\vec v_B)
\end{eqnarray}

In the case of collision of the object A with a plane B or other object of infinite mass
\begin{equation}
\vec v'_A = \lim_{m_B-\rightarrow \infty} \vec v_A + \frac{m_B}{m_A+m_B}(c+1)(\vec v_B-\vec v_A) = \vec v_A + (c+1)(\vec v_B-\vec v_A)
\end{equation}
